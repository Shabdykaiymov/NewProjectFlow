...
class AnalyticsService:
    ...
    def get_tasks_by_user(project_id=None, user=None):
        ...
        # Генерируем случайные цвета для каждого пользователя
        import random

        for user_item in user_counts:
            # Пропускаем записи с None
            if not user_item['assignee__username']:
                continue

            # Формируем имя пользователя
            name = user_item['assignee__first_name']
            if user_item['assignee__last_name']:
                name = f"{name} {user_item['assignee__last_name']}".trim()
            if not name:
                name = user_item['assignee__username']
